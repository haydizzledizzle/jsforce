    ...
    var pollBatches = function() {
      job.list(function(err, batches) { 
        if (!err) {
          ...
          if (allCompleted) {
            // https://github.com/jsforce/jsforce/issues/764
            // If sforce gets no records in one of the pkchunk batches it just puts "Records not found for this query" in the csv
            // which breaks parsing, ignore these
            retrieveAll(batches.filter(b => !(b.numberRecordsFailed == '0' && b.numberRecordsProcessed == '0')));
          } else {
